...
function getCartesianProduct(values: any) {
	...
	const formattedValues = values
		...
		.filter((f: {name: string}[]) => {
			...
			return f.find(e => !NAME_EXTRAS.test(e.name.toLocaleLowerCase()))
		});
			...
			<Card className="w-full sm:w-8/12 md:w-2/3 p-0 md:p-0">
				<div className="border-t border-dashed border-gray-200 mb-5 md:mb-8">
					...
					{!!custom_variation && custom_variation[0] && custom_variation[0].attribute &&
						<div className="pt-5">
							<Title className="text-lg mt-5 uppercase text-center px-5 md:px-8 mb-0 border-t border-dashed border-gray-200 pt-10 w-100">
								Preços
							</Title>
							<div className="flex border-t mt-5 w-full flex-col  border-dashed border-gray-200 last:border-0 p-5 md:p-8">

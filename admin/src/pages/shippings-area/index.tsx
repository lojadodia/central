...
export default function ShippingsAreaPage() {
  ...
  useEffect(() => {
    if (data?.shippingClasses) {
      ...
      let data_transform = data.shippingClasses.filter((d: any) => d.polygon !== null).map((p: any) => p.polygon).map((json) => json && JSON.parse(json))
      if (data_transform) {
    ...
    <>
      <Card className="flex flex-col md:flex-row items-center mb-8">
        <div className="md:w-1/4 mb-4 md:mb-0">
          <h1 className="text-xl font-semibold text-heading">Areas de Entregas</h1>
        </div>
      </Card>

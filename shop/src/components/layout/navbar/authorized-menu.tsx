        ...
        <AnimatePresence>
          {isOpen && (
            <motion.ul
              ...
              className="py-4 w-48 bg-white dark:bg-neutral-800 rounded shadow-700 z-20"
            >
              {siteSettings.authorizedLinks.map(({ href, label }) => (
                <li key={`${href}${label}`}>
                  <button
                    onClick={() => handleClick(href)}
                    className="block w-full py-2.5 px-6 text-sm text-left dark:text-gray  capitalize text-heading transition duration-200 hover:text-primary focus:outline-none"
                  >
                    {label}
                  </button>
                </li>

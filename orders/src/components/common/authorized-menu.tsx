...
export default function AuthorizedMenu() {
  ...
  if(permission == "super_admin" || permission == "admin"){
    var MENU_BANNERS : any = { href: ROUTES.BANNERS, label: "Cartazes" };
  }else{
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.ul
              ...
              className="py-2 w-48 bg-white dark:bg-neutral-900  rounded shadow-base z-50"
            >
              {site.authorizedLinks.map(({ href, label }) => (
                ...
                >
                  {href && (
                  <Link href={href}>
                    <a className="block px-4 py-3 text-sm text-heading dark:text-white font-medium transition duration-200 hover:text-primary  ">
                      {label}
                    </a>
                  </Link>
                  )}

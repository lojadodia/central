...
export default function Uploader({ onChange, value, multiple }: any) {
  ...
  const thumbs = files?.map((file: any, idx) => {
    if (file.id) {
      return (
        <div
          className="inline-flex flex-col overflow-hidden border border-gray-200 dark:border-neutral-600 rounded mt-2 mr-2 relative"
          key={idx}
        >
  ...
  return (
    <section className="upload">
      <div
        {...getRootProps({
          className:
            "border-dashed border-2 border-gray-300 dark:border-neutral-500 h-36 rounded flex flex-col justify-center items-center cursor-pointer focus:border-gossamer-400 focus:outline-none",
        })}
      >
        <input {...getInputProps()} />

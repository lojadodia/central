...
const When = () => {
  ...
  const { total, isEmpty, addItemToCart, clearItemFromCart, isInCart } =
    useCart();
  ...
  useEffect(() => {
    axios.get(url + "ebb1d3d715/" + base64id).then(function (response) {
  ...
  const currentTime =
    settings?.schedule[moment(Date.now()).format("dddd").toLocaleLowerCase()];
  ...
  useEffect(() => {
    if (isAuthorize) {
      ...
      if (
        ...
        settings?.order?.type?.takeaway == "true" &&
        settings?.order?.type?.delivery == "false"
      ) {
        setOrderType("takeaway");
      } else if (
        settings?.order?.type?.takeaway == "false" &&
        settings?.order?.type?.delivery == "true"
      ) {
  ...
  function isDateValid() {
    ...
    const interval =
      order_type != "takeaway"
        ? settings?.schedule[_weekName]?.interval
        : settings?.schedule[_weekName]?.interval2;
    // Limites
    ...
    // Subtrair Minutos
    const endtime1 = interval
      ? moment(date1).subtract(interval, "minutes").format("HH:mm")
      : end1;
    const endtime2 = interval
      ? moment(date2).subtract(interval, "minutes").format("HH:mm")
      : end2;
    // Filtrar
    let currentDayWeek =
      settings.schedule[moment(settings.now).format("dddd").toLocaleLowerCase()]
        .value;
    return (
      ((currentHour >= currentTime?.time1?.start && currentHour <= endtime1) ||
        (currentHour >= currentTime?.time2?.start &&
          currentHour <= endtime2)) &&
      currentDayWeek
    );
  }
  var date = new Date();
  ...
  function handlerTime(time: any) {
    ...
    const datetime = moment(
      selectedDate + " " + time?.target.value,
      "DD-MM-YYYY HH:mm"
    );
  ...
  function createSequenceOfTimesDelivery(
    ...
    for (let i = 0; i < 180; i++) {
      if (
        time_aux.hour() < hourStart1 ||
        time_aux.hour() > parseInt(hourEnd1, 10)
      )
        continue;

      if (
        time_aux.hour() == parseInt(hourEnd1, 10) &&
        time_aux.minute() > parseInt(minuteEnd1, 10)
      )
        continue;
  ...
  return (
    <>
      <div className="mb-5">
        ...
        {order_type == "delivery" && (
          <div className="relative mt-4">
            <div
              className={`shadow-700  rounded-lg  ${
                location.hash == "#target-address" &&
       ...
       */}
          <div
            onClick={() => {
              optionChecked("schedule", "click");
            }}
            className={`px-4 py-3 text-center text-sm  rounded  ${
              checkSchedule ? "bg-primary h-12 text-white" : ""
            } border-gray-200 border dark:border-neutral-700 cursor-pointer`}
          >
            <RiCalendar2Fill
              style={{ display: "inline-block", verticalAlign: "-2px" }}
            />{" "}
            AGENDAR {order_type != "takeaway" ? "" : ""}
          </div>
          ...
          {checkSchedule ? (
            <div className="w-full">
              <div className="relative">
                ...
                <button
                  className="px-2 w-full py-3 mb-5 text-center text-sm mr-4  rounded  bg-black  h-12 text-white border-gray-200 border dark:border-neutral-700 cursor-pointer"
                ...
                {isOpen && (
                  ...
                  >
                    <DatePicker
                      ...
                      inline
                      className={"px-3 h-12 w-12 text-center rounded outline-none dark:bg-black dark:text-white dark:border-neutral-700  appearance-none transition duration-300 ease-in-out text-heading text-sm dark:text-white focus:outline-none focus:ring-0 border border-gray-300 focus:border-primary".concat(
                        startDate ? "" : ""
                      )}
                    />
                  </div>
              ...
              {weekName && !!includeTimes.length && (
                <div className="relative">
                  ...
                  <input
                    type="time"
                    placeholder="Hora"
                    className="px-3 py-3 text-center text-sm  rounded  bg-yellow-400  h-12 w-full text-black border-gray-200 border dark:border-primary cursor-pointer text-center"
        ...
        {isDateValid() ? (
          <>
            {timeInterval > 0 && (
              <>
                <div
                  className="bg-yellow-100 mt-5 border-t-4 dark:text-neutral-800 border-yellow-300 rounded-b text-teal-900 px-4 py-3 shadow-md"
                  role="alert"
                ...
                >
                  <div className="flex">
                    <div className="py-1">
                      <RiTimeLine
                        className="fill-current h-7 w-7 text-teal-500 mr-4"
                        style={{
                    ...
                    <div>
                      <p className="uppercase pt-2">
                        Tempo de{" "}
                        {order_type == "delivery" ? "Entrega " : "Preparação "}{" "}
                        <b> {timeInterval}min</b>
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <>
            <div
              className="bg-yellow-100 dark:bg-yellow-400  dark:text-neutral-800 dark:border-yellow-600  mt-5 border-t-4 border-yellow-300 rounded-b text-teal-900  dark:text-black px-4 py-3 shadow-md"
              role="alert"
            >
              <div className="flex">
                <div className="py-1">
